...
inputFile1 = LOAD 'hdfs:///user/anvita/episodeIV_dialogues.txt' using PigStorage('\t') AS (name,line);
inputFile2 = LOAD 'hdfs:///user/anvita/episodeV_dialogues.txt' using PigStorage('\t') AS (name,line);
inputFile3 = LOAD 'hdfs:///user/anvita/episodeVI_dialogues.txt' using PigStorage('\t') AS (name,line);
...
STORE Sorted_Output INTO 'hdfs:///user/anvita/Output_Characters';
